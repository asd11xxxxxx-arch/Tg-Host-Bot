...
# Webhook URL setup (Replace with your Render URL)
...
@app.route('/')
def index():
    return "VORTEXA ULTIMATE CLOUD v12.0 - Bot is Running!", 200
...
def health_check():
    return {
        ...
        'total_processes': sum(len(procs) for procs in running_processes.values()),
...
def setup_webhook():
    ...
    if result:
        ...
        print(f"❌ Failed to set webhook: {webhook_url}")
...
if __name__ == "__main__":
    ...
    if os.environ.get('RENDER'):
        print("✅ Running on Render - Setting up webhook...")
        ...
        if setup_webhook():
            ...
            print("⚠️ Webhook setup failed, falling back to polling mode")
            # Fallback to polling
            while True:
                try:
